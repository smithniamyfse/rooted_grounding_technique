...
router.get("/", rejectUnauthenticated, (req, res) => {
    pool.query(ALL_ENTRIES_QUERY, [req.user.id])
    .then(result => {
...
router.get("/top-see-items", rejectUnauthenticated, (req, res) => {
    getTopSeeItems(req.user.id)
    .then(topSeeItems => {
...
router.get("/top-triggers", rejectUnauthenticated, (req, res) => {
    getTopThreeTriggers(req.user.id)
    .then(topTriggers => {
...
router.get("/:id", rejectUnauthenticated, (req, res) => {
  pool.query(SPECIFIC_ENTRY_QUERY, [req.params.id, req.user.id])
  .then(result => {
      res.send(result.rows);
  })
  .catch(error => {
