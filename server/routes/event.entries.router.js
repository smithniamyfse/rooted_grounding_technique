...
router.post("/new", rejectUnauthenticated, async (req, res) => {
    ...
    const queryText = 'INSERT INTO "user_event_entries" ("location", "date", "time", "user_id") VALUES (NULL, CURRENT_DATE, CURRENT_TIME, $1) RETURNING "id";';
    try {
...
router.get("/:id", rejectUnauthenticated, async (req, res) => {
    try {
        const result = await pool.query(SPECIFIC_ENTRY_QUERY, [req.params.id, req.user.id]);
        res.send(result.rows);
    } catch (error) {
...
router.put("/:id", rejectUnauthenticated, async (req, res) => {
    ...
    try {
        await pool.query(queryText, [req.body.location, req.params.id, req.user.id]);
        res.sendStatus(200);
    } catch (err) {
